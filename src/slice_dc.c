...
* @param method Charater array defining the method to be used for binning
* @param window_size Total width of each vector that will be returned within the list
...
* @todo Nothing
*/
SEXP slice_dc(SEXP gen_ind, SEXP l_ind, SEXP scores, SEXP start, SEXP end, SEXP window_count, SEXP method) {
	int upcounter=0,scores2write=0,i=0,j=0,k,offset2score,maxindex,slicen;
	int minindex=0,blscore_end=0,rebuildc=0,tofirst,upc_inner=0;

	signal(SIGINT,SIG_DFL);//make this thing stop on CTRL+C

	SEXP slice,dense_list,new_vec;
	...
	for(slicen=0;slicen<total_queries;slicen++){
		...
		PROTECT(slice = NEW_INTEGER(scores2write));upc_inner++;
		int *slicep= INTEGER_POINTER(slice);
...
//		}
		...
		if(wsize && rebuildc!=wsize){
			if(rebuildc>wsize){
				PROTECT(new_vec = NEW_INTEGER(wsize));upc_inner++;
				int *new_vecp= INTEGER_POINTER(new_vec);
				shrink(slicep,new_vecp,rebuildc,wsize,summarizep);
				SET_VECTOR_ELT(dense_list, slicen, new_vec);
			}else if(rebuildc<wsize){
				PROTECT(new_vec = NEW_INTEGER(wsize));upc_inner++;
				int *new_vecp= INTEGER_POINTER(new_vec);
		...
		UNPROTECT(upc_inner);//unprotect here instead of adding to upcounter to prevent PROTECT stack overflow for queries using many regions
	}
